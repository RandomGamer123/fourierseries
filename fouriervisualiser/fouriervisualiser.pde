...
int[] midadj = {190,100}; //represents an offset to the center
JSONArray coefficients;
float[] startvalue = new float[2];
Pointer[] pointerarr = new Pointer[200]; //edit this to be equal to the number of pointers
float scale = 1; //represents the amount you want to scale the visualisation up by
float baseangleincrement = 0.005; //represents the speed
float[] pointsx = new float[1400]; //edit this to be larger than TAU/baseangleincrement
float[] pointsy = new float[1400];
int pointCount = 0;
void setup() {
  size(1200,1200);
  background(0);
  ...
  //System.out.println("SVX: " + startvalue[0]);
  //System.out.println("SVY: " + startvalue[1]);
}
void draw() {
  background(0);
  float[] currentloc = new float[2];
